...
* Если мы изначально хотим заложить возможность фильтрации данных по фамилии или имени, 
то Фамилия, Имя и Отчество должны храниться в базах данных отдельно.

Данные о статусе (семейном положении) обычно хранятся не текстом, а с помощью
идентификаторов (в отдельном справочнике) →
1 — холост, 2 — женат, 3 — разведён.
Но в этом задании мы используем текстовые значения статуса.
...
* Лучше сразу рассчитать максимальное необходимое количество памяти для каждого типа данных и закладывать
достаточное количество памяти.
* Лучше выносить города в отдельную таблицу (справочник).
* Это избавит от проблем с опечатками:
система не будет думать, понимать ли под значениями «москва» и «Москва» один город, вместо этого оператор выберет значение из списка.
...
Например, можно разнести значения из «ФИО», номера телефонов и т.д.
...
Многое ко многому
* По одному адресу зарегистрированы несколько людей, и у каждого человека, в свою
очередь, может быть несколько адресов: фактический и адрес регистрации, например.

В примере разбора базы данных «Аренда автомобилей» - 
* Ключ (в нашем случае, Car_ID) — уникальный идентификатор. 
Для одного номера есть только одна машина. 

Одну и ту же машину в разные моменты времени могут брать в аренду разные люди.
Как в этой ситуации хранить данные о клиенте:
* Затирать данные о предыдущем клиенте новым → тогда в базе данных не будет информации о предыдущих клиентах. 
Это важно, если у машины обнаруживается поломка и необходимо установить, кто брал её в аренду.
...
Кейс: штраф
Компания, которая сдаёт машины в аренду, получает штраф за превышение скорости.
Компании необходимо установить, кто из клиентов брал машину в момент, когда был выписан штраф.

1. Изначально в таблице не было поля «Регистрационный номер», и сейчас имеющейся информации недостаточно,
чтобы установить, кто из клиентов превысил скорость и кто должен платить штраф.
...
ответ:  Reg_num         text   16   Гос номер

3. Теперь можно найти клиента, которому выписали штраф!
Далее из штрафа берём номер машины, находим его идентификатор, находим клиентов, которые брали машину в аренду. 
Далее ограничиваем время аренды: начало сделки меньше времени получения штрафа, конец сделки — больше. 
Таким образом находим идентификатор клиента.

Как сделать оптимальную базу данных:
* Изначально в таблице не было поля «Регистрационный номер», и сейчас имеющейся информации недостаточно, чтобы установить, 
кто из клиентов превысил скорость и кто должен платить штраф.
